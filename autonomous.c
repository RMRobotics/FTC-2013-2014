...
task main() {
	...
	//waitForStart();
	state.lastGyroCheckTime = time1[T2];
...
void getSensors(State *state){
	...
	//wait1Msec(20);
	state->lastGyroCheckTime += 20;
	int waitTime = state->lastGyroCheckTime-time1[T2];
	if (waitTime >= 0 && waitTime <= 20) {
		...
		eraseDisplay();
			string errorDisplay;
			sprintf(errorDisplay, "missed %d",(int)(time1[T2] - state->lastGyroCheckTime));
