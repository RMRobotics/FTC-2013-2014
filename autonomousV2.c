...
task main() {
	...
	while(state.currentState == LINEFOLLOWER1){
		...
		}else if(state.dist < 35){

			state.leftSpeed  = DRIVESPEED*LINEFOLLOWRATIO*1.5;
			state.rightSpeed = DRIVESPEED;

		}else if(state.dist > 40){
	...
	while(state.currentState == LINEFOLLOWER2){
		...
		/*if(state.irDir == 5){
			...
			state.leftSpeed = DRIVESPEED;
			state.rightSpeed = DRIVESPEED*LINEFOLLOWRATIO*1.5;
			numTime50 = 0;
	...
	while(state.currentState == PARKSEQUENCE1){
		...
		if(abs(state.degrees) >= 45){// || time1[T1] >= 500){
			stopMotors();
